...
/**
 * to represent the foreign key of a model template
 * Class ModelTemplateForeignKey
 * @package app\components\templates
 */
class ModelTemplateForeignKey extends Model
{
    /**
     * must be unique
    ...
    public function generateForeignKey(Migration $migration = null)
    {
        ...
        try {
            ...
            $migration->addForeignKey($name, $this->_model->table, $this->localColumns, $this->foreignTable, $this->foreignColumns, "RESTRICT", "CASCADE");
        } catch (Exception $dbe) {
    ...
    public function validateDatabaseStructure (& $warnings, $migration = null)
    {
        ...
        if (!$foreignKey) {
            ...
            if ($migration) $migration->addForeignKey($this->getShortenedName(), $this->_model->table, $this->localColumns, $this->foreignTable, $this->foreignColumns, "RESTRICT", "CASCADE");
        }
        else {
            ...
            for($i=0; $i<sizeof($localColumns); $i++) {
                if ($this->localColumns[$i] != $localColumns[$i]) {
                    ...
                    if ($migration) {
                        $migration->dropForeignKey($name, $this->_model->table);
                        $migration->addForeignKey($this->getShortenedName(), $this->_model->table, $this->localColumns, $this->foreignTable, $this->foreignColumns, "RESTRICT", "CASCADE");
                    }
                    break;
                }
            }
