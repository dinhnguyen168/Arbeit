...
<td class="logo logo1">
  ...
  <tr>
    ...
    <td class="inner">
        <div class="hole">Depth [m]:</div>
          <div class="value"><?= round($firstSection->top_depth + $firstSection->mcd_offset, 2) ?> to <?= round($lastSection->top_depth + $lastSection->mcd_offset + $lastSection->section_length, 2) ?></div>
        <div class="hole">Sections:</div>
          <div class="value"><?= getSectionCombinedIdPart($firstSection) ?> to <?= getSectionCombinedIdPart($lastSection) ?></div>

    </td>
    <td class="marker">
      <?php if ($topBottom == "bottom"): ?>
        <svg height="18mm" width="18mm">
          <circle cx="50%" cy="50%" r="45%" stroke="black" stroke-width="2" fill="white" />
          <text x="50%" y="50%" text-anchor="middle" stroke="black" stroke-width="0.1mm" dy="2.7mm" font-size="8.7mm">B</text>
        </svg>
      <?php else: ?>
        <svg height="18mm" width="18mm">
          <circle cx="50%" cy="50%" r="45%" fill="black" />
          <text x="50%" y="50%" text-anchor="middle" fill="white" stroke="white" stroke-width="0.3mm" dy="2.6mm" font-size="8.2mm">T</text>
        </svg>
      <?php endif; ?>
    </td>
  </tr>
